...
(async () => {
  ...
  const files = (await readdir('./src/react-spectrum-workflow/dist')).filter(item => !['index.js', 'util.js'].includes(item));

  const iconTalbe =
`${files.reduce(
  (pre, cur) => pre + `import { A4u${basename(cur, '.js')} } from './react-spectrum-workflow/dist/${basename(cur, '.js')}';\n`, '')}
export default () =>
  <table>
    <thead>
      <tr>
